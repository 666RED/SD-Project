...
public class WeeklyReportFrame extends JFrame{
	...
	WeeklyReportFrame() {
		...
		lblWeeklyNav.setFont(selectedText);
	...
	public void retrieveData(JScrollPane spData) {
		...
		try {
			...
			String sql = "SELECT r.receipt_date AS 'DATE', SUM(r.receipt_total) AS 'TOTAL PRICE', SUM(r.total_cost) AS 'TOTAL COST', SUBSTRING(r.receipt_date, 9, 2) AS 'DAY' "
					   + "FROM ( "
			...
			if(rs != null) {
				...
				double[] priceList = {0, 0, 0, 0, 0};
				double[] costList = {0, 0, 0, 0, 0};
				...
				while(rs.next()) {
					...
					if(day >= 1 && day <= 7) {
						priceList[0] += rs.getDouble("TOTAL PRICE");
						costList[0] += rs.getDouble("TOTAL COST");
					}else if(day >= 8 && day <= 14) {
						priceList[1] += rs.getDouble("TOTAL PRICE");
						costList[1] += rs.getDouble("TOTAL COST");					
					}else if(day >= 15 && day <= 21) {
						priceList[2] += rs.getDouble("TOTAL PRICE");
						costList[2] += rs.getDouble("TOTAL COST");					
					}else if(day >= 22 && day <= 28) {
						priceList[3] += rs.getDouble("TOTAL PRICE");
						costList[3] += rs.getDouble("TOTAL COST");					
					}else if(day >= 29 && day <= 31) {
						priceList[4] += rs.getDouble("TOTAL PRICE");
						costList[4] += rs.getDouble("TOTAL COST");
					}
				}
				...
				for(int i = 0; i < end; i++) {
					double profit = priceList[i] - costList[i];
					
					final JPanel panDataRow;
					JLabel lblNo;
					final JLabel lblWeek;
					...
					String strCost = "RM " + String.format("%.2f", costList[i]);
					String strPrice = "RM " + String.format("%.2f", priceList[i]);
					...
					if(i == 0) {
						startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-01"; 
						endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-07"; 
					}else if(i == 1) {
						startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-08"; 
						endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-15"; 
					}else if(i == 2) {
						startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-16"; 
						endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-21"; 
					}else if(i == 3) {
						startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-22"; 
						endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-28"; 
					}else if(i == 4) {
						if(month.equals("02")) {
							startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-29"; 
							endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-29"; 
						}else {
							String[][] weekInterval = determineWeekInterval();
							startDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-" + weekInterval[0][0];
							endDate = yearChooser.getYear() + "-" + (monthChooser.getMonth() + 1) + "-" + weekInterval[0][1];
						}
					}
					...
					lblWeek = new JLabel("  " + startDate + " -> " + endDate);
					lblWeek.setBounds(100, 0, 610, 43);
					lblWeek.setFont(regularText);
					lblWeek.setForeground(Color.BLACK);
					lblWeek.setBorder(BorderFactory.createMatteBorder(0, 1, 0, 1, BORDER_COLOR));
					lblWeek.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
					lblWeek.addMouseListener(new MouseListener() {
						...
						public void mousePressed(MouseEvent e) {
							dispose();
							new WeeklySalesFrame(strStartDate, strEndDate);
						}
	...
	public String[][] determineWeekInterval() {
